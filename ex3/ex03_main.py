...
class MCMCSampler:
    def __init__(self, model, img_shape, sample_size, num_classes, cbuffer_size=256):
        ...
        # Initialize replay buffer for each class
        self.replay_buffer = {i: [] for i in range(num_classes)}
    ...
    def synthesize_samples(self, clabel=None, steps=60, step_size=10, return_img_per_step=False):
        ...
        :param clabel: Class label(s) used to sample the buffer (can be a tensor of labels)
        ...
        if clabel is not None:
            if isinstance(clabel, torch.Tensor):
                clabel = clabel.detach().cpu()
                batch_size = len(clabel)
            else:
                batch_size = self.sample_size
        else:
            batch_size = self.sample_size

        # Initialize samples using reservoir sampling strategy
        # Mix 5% random noise with 95% from replay buffer
        n_random = max(1, int(0.05 * batch_size))
        n_from_buffer = batch_size - n_random

        # Create label list for each sample
        init_labels = []

        # Generate random samples with corresponding labels
        random_samples = torch.randn(n_random, *self.img_shape) * 0.5
        buffer_samples = []

        imgs_needed = n_from_buffer

        if clabel is not None:
            clabel_list = clabel.tolist()

            for i in range(imgs_needed):
                label_idx = clabel_list[i % len(clabel_list)]
                init_labels.append(label_idx)

                class_buffer = self.replay_buffer[label_idx]

                if len(class_buffer) > 0:
                    rand_idx = torch.randint(0, len(class_buffer), (1,)).item()
                    buffer_samples.append(class_buffer[rand_idx])
                else:
                    buffer_samples.append(torch.randn(*self.img_shape) * 0.5)

        else:
            unconditional_buffer_key = 0
            buffer = self.replay_buffer[unconditional_buffer_key]

            n_from_buffer_actual = min(imgs_needed, len(buffer))

            if n_from_buffer_actual > 0:
                indices = torch.randint(0, len(buffer), (n_from_buffer_actual,))
                buffer_samples.extend([buffer[i] for i in indices])

            if n_from_buffer_actual < imgs_needed:
                n_extra = imgs_needed - n_from_buffer_actual
                buffer_samples.extend(
                    torch.randn(n_extra, *self.img_shape) * 0.5
                )

        # --- final batch ---
        if buffer_samples:
            buffer_samples = torch.stack(buffer_samples)
            inp_imgs = torch.cat([random_samples, buffer_samples], dim=0)
        else:
            inp_imgs = random_samples
        ...
        for _ in range(steps):
            # (1) Add small noise to the input (Brownian motion)
            noise = torch.randn_like(inp_imgs) * 0.01
            inp_imgs.data.add_(noise).clamp_(-1.0, 1.0)

            # (2) Calculate gradient-based score function
            ...
            # (3) Perform gradient ascent (or descent on energy surface)
        ...
        ### Update replay buffer with synthesized samples
        imgs = inp_imgs.detach()

        if clabel is not None:
            # Class-conditional replay buffer update
            for img, label_idx in zip(imgs, init_labels):
                class_buffer = self.replay_buffer[label_idx]

                if len(class_buffer) < self.cbuffer_size:
                    class_buffer.append(img.cpu())
                else:
                    replace_idx = torch.randint(0, self.cbuffer_size, (1,)).item()
                    class_buffer[replace_idx] = img.cpu()

        else:
            # Unconditional replay buffer (shared, no class semantics)
            unconditional_buffer_idx = 0
            unconditional_buffer = self.replay_buffer[unconditional_buffer_idx]

            for img in imgs:
                if len(unconditional_buffer) < self.cbuffer_size:
                    unconditional_buffer.append(img.cpu())
                else:
                    replace_idx = torch.randint(0, self.cbuffer_size, (1,)).item()
                    unconditional_buffer[replace_idx] = img.cpu()
